...
#' 
#' ## Random Forests
#' We will briefly take a look at random forests, using the `randomForest` package. A random forest is a collection of slightly randomized decision trees (hence the name "forest"), and can be used for classification or prediction. They often have excellent predictive performance, but can be expensive to train and lack interpretability. Random forests have many hyperparameters that can be tuned to achieve the best possible predictive performance. Perhaps the most important hyperparameter is the number of trees to include in the forest. More trees results in a longer training time but can improve prediction and decrease overfitting. Other parameters can be seen by inspecting the randomForest command:
## -----------------------------------------------------------------------------
?randomForest

#' 
#' Let's start by training a random forest model for a classification task. We will perform the same task of predicting whether or not a listing has an elevator, using price and neighborhood as predictors. We will compare the performance of random forest to what we got using our simple CART model. We'll try two models, one using 5 trees and one using 100 trees.
## -----------------------------------------------------------------------------
set.seed(123)
...
#' 
#' Compare this to what we got using rpart, which had accTrain=0.8189944 and accTest=0.8031291. So random forest with only 5 trees does not do very well, but with 100 trees it is doing a bit better than CART! 
#' 
...
#' 
#' The prediction accuracy is now much better, and it doesn't look we are overfitting at all. It would likely be even better if the other hyperparameters of the random forest model were properly tuned! Although random forest models are not very interpretable and hard to visualize, there is a popular method called *variable importance* that is commonly used with random forest models. We will use the function `varImpPlot` form the `randomForest` package. This plot shows us which variables the random forest model has determined are the most important for predicting whether or not a listing contains an elevator. It uses a metric called `MeanDecreaseGini` by default, which is the mean decrease in node impurity (you can read more about this metric online), to rank the importance of all of the predictors used in the model. There are other metrics besides the Gini Importance that can also be used as a metric.
#' 
## -----------------------------------------------------------------------------
varImpPlot(rf)
